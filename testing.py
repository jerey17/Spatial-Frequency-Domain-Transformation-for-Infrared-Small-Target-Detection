...
class ROCMetric05():
    ...
    def get(self):
        ...
        FP = self.fp_arr / (self.neg_arr + self.pos_arr + 0.001)
...
parser.add_argument("--model_names", default=['FFNet'], type=list,
                    help="model_name: 'ACM', 'DNANet', 'ISNet', 'ACMNet', 'Ours01', 'ISTDU-Net', 'U-Net', 'RISTDnet'")
parser.add_argument("--pth_dirs", default=None, nargs='+',
                    help="checkpoint dir, default=None or ['FFNet_500.pth.tar']")
...
parser.add_argument("--save_img_dir", type=str, default='./result',
...
def test():
    test_set = TestSetLoader(opt.dataset_dir, opt.train_dataset_name, opt.test_dataset_name,opt.base_size, opt.img_norm_cfg)
    ...
    net = FFNet(in_channels=1, mode='test', deep_supervision=True).cuda()
    ...
    with torch.no_grad():
        for idx_iter, (img, gt_mask, size, img_dir) in enumerate(tbar):
            ...
            # x4,x3,x2,x1,_,final = net.forward(img)

            # save_dir = 'E:\\lyh\\NewTrainer\\HeatMap'

            # visualize_heatmap((x4,x3,x2,x1,final),save_dir,img_dir[0])
...
# def visualize_heatmap(preds,save_dir,img_name):
#     x4,x3,x2,x1,final = preds

#     layers = [x4,x3,x2,x1,final]
#     layers_titles = ['Decoder4','Decoder3','Decoder2','Decoder1','Final']

#     for i,layer in enumerate(layers):
#         plt.imshow(layer[0,0].cpu().detach().numpy(),cmap='viridis',interpolation='nearest')
#         plt.title(f'Heatmap - {layers_titles[i]} ')

#         img_path = os.path.join(save_dir,f'{img_name}_layer{i+1}.png')
...
if __name__ == '__main__':
    ...
    if opt.pth_dirs == None:
        for i in range(len(opt.model_names)):
            ...
            opt.f.write(opt.model_name + '_500_best(1).pth.tar' + '\n')
            for dataset_name in opt.dataset_names:
                ...
                opt.pth_dir = opt.save_log + opt.dataset_name + '/' + opt.model_name + '_500_best(1).pth.tar'
                test()
            print('\n')
            opt.f.write('\n')
        opt.f.close()
